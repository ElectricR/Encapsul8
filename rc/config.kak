# Declaration of user-tweakable options

declare-option -hidden str wrapify_mapping_action_select_inner # = 'i'
declare-option -hidden str wrapify_mapping_action_select_outer # = 'a'
declare-option -hidden str wrapify_mapping_action_delete # = 'd'
declare-option -hidden str wrapify_mapping_action_replace # = 'r'
declare-option -hidden str wrapify_mapping_wrap_shortcut # = 's'
declare-option -hidden str wrapify_mapping_cancel # = '<esc>'
